...
from dualmask.model import DualMaskLanguageModel  # 确保 DualMaskLM 在 PYTHONPATH


def main():
    parser = argparse.ArgumentParser(description="使用训练好的 DualMaskLanguageModel 进行推理。")
    parser.add_argument(
        "--model_path",
        type=str,
        required=True,
        help="包含模型权重、配置和tokenizer的已训练模型目录的路径 (例如 'outputs_dualmask_llama/final_model')"
    )
    parser.add_argument(
        "--text",
        type=str,
        default="This is an example sentence to test the DualMask model.",
        help="用于推理的输入文本。"
    )
    parser.add_argument(
        "--device",
    ...
    # 2. 加载模型配置 (可选，但可以用来验证)
    # model_config = DualMaskModelConfig.from_pretrained(args.model_path)
    # print(f"Model config loaded. Vocab size: {model_config.vocab_size}")

    # 3. 加载模型
    # 确保 DualMaskLanguageModel 类已注册或可被 from_pretrained 找到
    # 如果遇到 AutoModel 无法找到自定义类的问题，直接使用 DualMaskLanguageModel.from_pretrained
    try:
        model = DualMaskLanguageModel.from_pretrained(args.model_path)
        model.to(args.device)
        model.eval()  # 设置为评估模式
        print(f"Model loaded successfully from {args.model_path} and moved to {args.device}")
    except Exception as e:
        print(f"Error loading model: {e}")
        print("确保 DualMaskLM 包在PYTHONPATH中，并且模型路径正确。")
        return
    ...
    if "corrupted_input_ids_for_eval" not in outputs or "mlm_logits_for_eval" not in outputs:
        ...
        print("This inference script expects DualMaskLanguageModel.forward() to return these keys in eval mode.")
