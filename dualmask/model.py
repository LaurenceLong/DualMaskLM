...
class DualMaskLanguageModel(PreTrainedModel):
    config_class = DualMaskModelConfig # 关联自定义配置

    def __init__(self, config: DualMaskModelConfig):
        ...
        # 可以为其配置不同的层数或参数（如果需要，可以创建单独的config对象）
        self.mask_picker_decoder = LlamaDecoder(config) # 使用主config或修改后的config
        self.keep_logits_head = nn.Linear(config.hidden_size, 1) # 输出每个token被保留的logit

        # 4. Decoder-2: Pointer-Inserter (使用LlamaDecoder结构)
        # 此Decoder用于在骨架的基础上，决定在哪些"缝隙"中插入MASK token
        self.pointer_inserter_decoder = LlamaDecoder(config)
        ...
        # 5. 主Encoder: Masked Language Modeling (使用LlamaEncoder结构)
        ...
        # - (可选) 如果LLaMA组件内部有自己的输入嵌入层（不推荐，应共享），也需要绑定
        #   例如: self.mlm_encoder.set_input_embeddings(self.word_embeddings)
        #   但通常LLaMA组件会直接接收嵌入向量作为输入。
    ...
    def forward(
        ...
        for b_idx in range(batch_size):
            ...
            if current_skeleton_len > 0:
                ...
                avg_insertion_slot_probs = pointer_probs[b_idx, :current_skeleton_len, :].mean(dim=0) # (max_pos_embed + 1,)
            ...
            for slot_idx in range(num_available_slots): # num_available_slots == max_pos_embed + 1
                ...
                if skeleton_token_iterator < current_skeleton_len:
                    reconstructed_sequence_for_item.append(current_unpadded_skeleton_ids[skeleton_token_iterator].item())
                    skeleton_token_iterator += 1
            
            # 将列表转换为tensor，并确保长度为seq_length (截断或填充)
            if len(reconstructed_sequence_for_item) > seq_length:
                corrupted_ids_tensor = torch.tensor(reconstructed_sequence_for_item[:seq_length], device=device, dtype=torch.long)
            else:
                ...
                if corrupted_ids_tensor.size(0) < seq_length:
                    ...
                    padding_tensor = torch.full((num_padding_needed,), self.config.pad_token_id, device=device, dtype=torch.long)
