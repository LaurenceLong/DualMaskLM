...
# 从本地dualmask包导入组件
from dualmask.configs import DualMaskModelConfig
from dualmask.data import build_custom_tokenizer, collate_fn_for_dualmask
from dualmask.model import DualMaskLanguageModel
...
def parse_training_arguments():
    parser = argparse.ArgumentParser(description="从头训练 DualMaskLanguageModel (LLaMA-based)。")
    ...
    # 模型配置参数 (允许命令行覆盖部分默认值)
    # DualMaskModelConfig中定义的参数都可以通过命令行传入，例如 --vocab_size 50000
    ...
    # 路径和日志参数
    parser.add_argument("--output_dir", type=str, default="outputs_dualmask_llama", help="保存检查点、日志和最终模型的目录。")
    ...
    parser.add_argument("--tensorboard_log_name", type=str, default="dualmask_llama_training_logs", help="Tensorboard日志的名称。")

    # 可以添加更多DualMaskModelConfig中的参数，例如 --lambda_comp 0.6
...
def main():
    ...
    config_overrides = {
        ...
        if v is not None and hasattr(DualMaskModelConfig, k)
    }
    model_config = DualMaskModelConfig(**config_overrides)
    ...
    train_dataloader = DataLoader(
        ...
        shuffle=True,
        collate_fn=lambda batch_items: collate_fn_for_dualmask(
    ...
    # --- 7. 初始化模型 ---
    logger.info("从头初始化 DualMaskLanguageModel...")
    # 注意：这里是关键，DualMaskLanguageModel内部的LLaMA组件需要能正确地从头初始化
    model = DualMaskLanguageModel(model_config)
    logger.info(f"模型结构:\n{model}")
    ...
    if accelerator.is_main_process:
        ...
        if len(train_dataset) > 0: # 从训练集中取一个样本
            ...
            with torch.no_grad():
                ...
                # 注意：DualMaskLanguageModel的forward需要current_train_step，评估时可以设为max_steps
